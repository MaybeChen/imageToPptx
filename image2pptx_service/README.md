...
By default, `SEGMENT_ENGINE=auto` tries a local YOLO model first when one is present, then falls back to OpenCV if the YOLO model fails or emits no usable elements. On Windows, if logs show `WinError 126`/`WinError 127` while loading `torch\lib\shm.dll` or another Torch DLL, YOLO was selected but PyTorch could not load its native dependencies. The named DLL file may already exist; this error often means one of that DLL's dependent DLLs is missing from the loader search path or has an incompatible ABI/version. Repair/install Microsoft Visual C++ Redistributable 2015-2022. If YOLO already works in another environment, set `YOLO_PYTHON` to that environment's `python.exe`; otherwise run `poetry run python -m pip install --force-reinstall torch torchvision --index-url https://download.pytorch.org/whl/cpu` inside the service environment. The service prefers a `best.pt` file and searches `storage/models/yolo`, `models/yolo` inside the service directory, and repository-root `models/yolo`; `YOLO_MODEL_PATH` still overrides all project-local locations. `SEGMENT_ENGINE=yolo`, `SEGMENT_ENGINE=yolo11`, and `SEGMENT_ENGINE=yolo26` force the same Ultralytics adapter. Segmentation prints `[segment] ...` messages to stdout/logs so you can see whether YOLO was selected, which model was loaded, how many YOLO items were produced, and whether OpenCV fallback was used. For Windows Torch DLL loader failures, the error message also runs a best-effort local DLL import-table diagnostic so logs can show the missing direct dependency when Windows exposes enough information. When YOLO runs through the conversion pipeline, an annotated intermediate image is written to `output/yolo_detections.png` and exposed as `/download/{job_id}/yolo_debug`, showing the raw boxes that SAHI+YOLO produced before segment conversion and PPTX rendering. The default YOLO confidence threshold is `YOLO_CONF=0.01`; tune sliced inference with `SAHI_SLICE_WIDTH`, `SAHI_SLICE_HEIGHT`, `SAHI_OVERLAP_WIDTH_RATIO`, `SAHI_OVERLAP_HEIGHT_RATIO`, and `SAHI_DEVICE`, and tune detection/merge with `YOLO_CONF`, `YOLO_IOU`, and `YOLO_MAX_DET`. Model class names are mapped into manifest segment types such as `icon`, `image`, `chart`, `table`, `shape`, `line`, `arrow`, and `background`; train the model with those names (or common aliases like `logo`, `picture`, `rectangle`, `table`, and `connector`) for best results. `table` is preserved as its own image-backed asset type. `text_region`/`text` labels are ignored because OCR owns native text generation, and `background` detections are kept as a layer without suppressing foreground objects. For this PPT/image-to-PPTX use case, a custom-trained `*-seg` model on slide screenshots is more important than the base YOLO generation, but YOLO26 is the recommended default for new CPU deployments because it improves CPU/edge efficiency and small-object coverage over YOLO11. If the YOLO model is unavailable or you intentionally want the heuristic path, keep `SEGMENT_ENGINE=opencv` to use the built-in fallback.

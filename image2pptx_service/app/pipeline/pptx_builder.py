...
def _rgb(hex_color):
    h=(hex_color or '#000000').lstrip('#'); return RGBColor(int(h[0:2],16), int(h[2:4],16), int(h[4:6],16))
...
def build_pptx(manifest, job_root: Path, output_path: Path) -> Path:
    ...
    for e in manifest.elements:
        ...
        if e.type == 'text':
            tb=slide.shapes.add_textbox(Inches(x), Inches(y), Inches(w), Inches(h)); p=tb.text_frame.paragraphs[0]; run=p.add_run(); run.text=e.text or ''; run.font.size=Pt(e.style.get('font_size',14)); run.font.name=e.style.get('font_family','Arial'); run.font.color.rgb=_rgb(e.style.get('color','#1F2937')); run.font.bold=bool(e.style.get('bold',False))
        elif e.type == 'shape':
